...
allplayers.app.client.prototype.init=function(){allplayers.app.prototype.init.call(this);this.ehost=allplayers.app.getParam("ehost");this.container=this.options.getContainer();this.height=0;this.heightTimer=null;this.queue={};this.checkout=this.reg=null;this.type="registration";var a=this,c=null;jQuery(document).bind("DOMSubtreeModified",function(){c&&clearTimeout(c);c=setTimeout(function(){a.resize()},500)});jQuery.pm.bind("chromeMsgResp",function(c){if(c.hasOwnProperty("guid")&&a.queue.hasOwnProperty(c.guid))a.queue[c.guid](c.response)});
jQuery.pm.bind("getRegistration",function(c){a.reg=c;a.type="registration"});jQuery.pm.bind("getCheckout",function(c){a.checkout=c;a.type="checkout"});this.resize();jQuery.pm({target:window.parent,url:this.ehost,type:"clientReady"})};allplayers.app.client.prototype.sendMessage=function(a,c){var g=function(){return Math.floor(65536*(1+Math.random())).toString(16).substring(1)};a.guid=g()+g()+g();this.proxy.post({event:{name:"chromeMsg",data:a}});this.queue[a.guid]=c};
