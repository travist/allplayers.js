...
a.data.event:!1)switch(a.name){case "chromeMsgResp":if(a.data.hasOwnProperty("guid")&&c.queue.hasOwnProperty(a.data.guid))c.queue[a.data.guid](a.data.response);break;case "chromePluginReady":window.postMessage(a,"*");break;case "addStyle":if(a.data){a=a.data.replace(/[<>]/g,"");var b=jQuery('link[type="text/css"]');b.length&&(b=b.eq(b.length-1),b.after(jQuery(document.createElement("style")).attr({type:"text/css"}).append(a)))}}});this.resize();this.proxy.post({event:{name:"serverReady"}})};
