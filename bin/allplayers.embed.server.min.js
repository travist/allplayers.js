...
allplayers.embed.server.prototype.init=function(){allplayers.embed.prototype.init.call(this);this.container=this.options.getContainer();this.height=0;this.heightTimer=null;this.queue={};var c=this;this.proxy=new Porthole.WindowProxy(this.options.proxy);var b=null;jQuery(document).bind("DOMSubtreeModified",function(){b&&clearTimeout(b);b=setTimeout(function(){c.height=0;c.resize()},500)});this.options.isComplete()&&this.proxy.post({event:{name:"complete"}});this.proxy.addEventListener(function(a){if(a=
a.data.hasOwnProperty("event")?a.data.event:!1)switch(a.name){case "chromeMsgResp":if(a.data.hasOwnProperty("guid")&&c.queue.hasOwnProperty(a.data.guid))c.queue[a.data.guid](a.data.response)}});this.resize()};allplayers.embed.server.prototype.sendMessage=function(c,b){var a=function(){return Math.floor(65536*(1+Math.random())).toString(16).substring(1)};c.guid=a()+a()+a();this.proxy.post({event:{name:"chromeMsg",data:c}});this.queue[c.guid]=b};
allplayers.embed.server.prototype.resize=function(){jQuery('a[href^="https://www."]',this.container).each(function(){var a=$(this).attr("href");$(this).attr({href:a.replace(/https:\/\/www.(.*?).allplayers.com/,"https://platform.$1.allplayers.com")})});var c=this,b=function(){var a=c.container.outerHeight(!0);c.height!==a&&(c.height=a,c.proxy.post({height:c.height,event:{name:"init",height:c.height,id:window.location.hash}}));c.heightTimer&&clearTimeout(c.heightTimer);c.heightTimer=setTimeout(b,500)};
b()};
