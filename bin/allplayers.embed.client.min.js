...
"/register";break;case "forms":a+="/forms"}this.options.query.etyp=this.options.type;a:{for(var g in this.options.query){d=!1;break a}d=!0}if(!d){var a=a+"?",f;for(f in this.options.query)a+=f+"="+encodeURIComponent(this.options.query[f]),a+="&";a=a.replace(/\&$/,"")}}var a=a+(-1===a.search(/\?/)?"?":"&"),a=a+("esrc="+$.base64("btoa",window.location.href,!0)),a=a+("#"+c),e=jQuery(document.createElement("iframe")).attr({id:c,name:c,scrolling:"no",seamless:"seamless",width:"100%",height:"0px",src:a}).css({border:"none",
...
window.postMessage(a,"*");break;case "init":b.remove();e.height(a.height).attr("height",a.height+"px");break;case "complete":j.options.complete.call(j,a);break;case "serverReady":j.pluginReady&&j.proxy.post({event:{name:"chromePluginReady"}})}})};
