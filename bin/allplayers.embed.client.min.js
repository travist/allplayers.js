...
allplayers.embed.client.prototype.init=function(){allplayers.embed.prototype.init.call(this);this.pluginReady=!1;var c=jQuery(document.createElement("div")).css({background:"url("+this.options.spinner+") no-repeat",margin:"0 5px",paddingLeft:"20px"});c.append(this.options.loading);var b=this.context.attr("id")+"_iframe",a="";if(this.options.src)a=this.options.src;else{a=this.options.base+"/g/"+this.options.group;switch(this.options.type){case "registration":a+="/register";break;case "forms":a+="/forms"}this.options.query.embedtype=
this.options.type;if(!jQuery.isEmptyObject(this.options.query)){var a=a+"?",d;for(d in this.options.query)a+=d+"="+encodeURIComponent(this.options.query[d]),a+="&";a=a.replace(/\&$/,"")}}var a=a+("#"+b),e=jQuery(document.createElement("iframe")).attr({id:b,name:b,scrolling:"no",seamless:"seamless",width:"100%",height:"0px",src:a}).css({border:"none",overflowY:"hidden"});this.context.append(c);this.context.append(e);this.proxy=new Porthole.WindowProxy(this.options.proxy,e.attr("id"));var f=this;"undefined"!==
typeof window.postMessage&&window.addEventListener("message",function(a){switch(a.data.name){case "chromeMsgResp":f.proxy.post({event:a.data});break;case "chromePluginReady":f.pluginReady=!0}});this.proxy.addEventListener(function(a){if(a=a.data.hasOwnProperty("event")?a.data.event:!1)switch(a.name){case "chromeMsg":"undefined"!==typeof window.postMessage&&window.postMessage(a,"*");break;case "init":c.remove();e.height(a.height).attr("height",a.height+"px");break;case "complete":f.options.complete.call(f,
a);break;case "serverReady":f.pluginReady&&f.proxy.post({event:{name:"chromePluginReady"}})}})};
